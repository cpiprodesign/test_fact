                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="form-group" :class="{'has-danger': errors.parent_id}">
                            ...
                            <el-select v-model="form.parent_id" :disabled="form.childrens.length" >
                                <el-option v-for="option in item_category_parents" :key="option.id" :value="option.id" :label="option.description"></el-option>
                            </el-select>
                            <small class="form-control-feedback" v-if="errors.parent_id"
            ...
            create() {
                this.titleDialog = (this.recordId) ? 'Editar Categoría' : 'Nueva Categoría'
                    this.$http.get(`/${this.resource}/tables`)
                        .then(response => {
                            this.item_category_parents = response.data.parents
                        })
                if (this.recordId) {
